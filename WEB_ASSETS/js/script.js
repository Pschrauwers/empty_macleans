...
$(document).ready(function(){
  ...
  var Jszoom;
  $(function() {
    return window.jszoom = new Jszoom;
  });
  Jszoom = (function() {
    function Jszoom() {
      $(document).on("click", ".js-zoom", function() {
        var $el, $wrapper;
        $el = $(this);
        if ($el.closest('.image-wrapper').length === 0) {
          $el.wrap('<div class="image-wrapper" />');
        }
        $wrapper = $el.parent();
        if ($wrapper.hasClass('js-slide')) {
          return $wrapper.removeClass('js-slide');
        } else {
          return $wrapper.addClass('js-slide');
        }
      });
    }
